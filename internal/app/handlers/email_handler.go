...
// EmailServiceInterface defines the interface for EmailService
type EmailServiceInterface interface {
	SendAsync(message utils.EmailMessage) error
	SendSync(message utils.EmailMessage) error
	SendBulk(messages []utils.EmailMessage) error
	QueueSize() int
...
func (h *EmailHandler) SendBulkEmail(c *gin.Context) {
	...
	messages := make([]utils.EmailMessage, 0, len(req.Emails))
